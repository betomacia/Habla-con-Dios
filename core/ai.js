// core/ai.js — Motor OpenAI + memoria (seguimiento) + lectura bíblica continua
...
function countWords(s = "") { return (s.match(/\b[\p{L}\p{N}’'-]+\b/gu) || []).length; }
...
function wantsBibleStart(s = "") {
  return /(leer la biblia|quiero leer la biblia|leemos la biblia|empezar lectura b[íi]blica)/i.test(s || "");
}
function wantsBibleContinue(s = "") {
  return /(continuemos|seguir leyendo|continuar lectura|retomar lectura)/i.test(s || "");
}
...
async function askLLM({ persona, message, history = [], userId = "anon", memoryIO, personaExtra = "" }) {
  ...
  // Frame/tema bloqueado si aplica
  ...
  // Lectura bíblica
  let bibleRefForced = "";
  if (wantsBibleStart(message)) {
    ensureBible(mem);
    mem.bible.index = 0;
    bibleRefForced = nextBibleRef(mem);
  }
  if (wantsBibleContinue(message)) {
    bibleRefForced = nextBibleRef(mem);
  }

  // Ventana de uso (budget de palabras habladas en 24h)
  mem.usage = mem.usage || { window_start: Date.now(), spoken_words_24h: 0 };
  const now = Date.now();
  const DAY = 24 * 60 * 60 * 1000;
  if (!mem.usage.window_start || now - mem.usage.window_start > DAY) {
    mem.usage.window_start = now;
    mem.usage.spoken_words_24h = 0;
  }
  ...
  // Sanitización
  let msg = limitWords(stripQuestionsFromMessage(normalize(data.message)), cfg.MAX_MSG_WORDS || 60);
  let ref = cleanRef(normalize(data?.bible?.ref));
  let text = normalize(data?.bible?.text);
  let question = normalize(data?.question);

  // Forzar referencia si hay plan en curso
  if (bibleRefForced) ref = bibleRefForced;

  // Validación de pregunta (evitar repetición / no en ACK)
  ...
  // Gobernadores de costo/ventanas
  if (cfg.VIDEO_WINDOW_EVERY_N_TURNS > 0) {
    mem.turns = mem.turns || 0;
    mem.turns += 1;
    if ((mem.turns % cfg.VIDEO_WINDOW_EVERY_N_TURNS) === 0) {
      msg = limitWords(msg, cfg.VIDEO_WINDOW_MAX_WORDS);
    }
  }

  // Budget diario de palabras habladas
  let words = countWords(msg);
  if (cfg.SPOKEN_WORDS_BUDGET_24H > 0 && (mem.usage.spoken_words_24h + words) > cfg.SPOKEN_WORDS_BUDGET_24H) {
    msg = "Respira lento unos segundos; no estás solo. Estoy contigo cuando lo necesites.";
    words = countWords(msg);
    question = "";
  }
  mem.usage.spoken_words_24h += words;

  // ACK_MODE: cierre corto
  if (ACK_MODE) {
    const closeTip = "Antes de cerrar, respira lento un minuto y repite: “no estoy solo”. Estoy contigo cuando lo necesites.";
    msg = limitWords(stripQuestionsFromMessage(msg || closeTip), Math.min(30, cfg.VIDEO_WINDOW_MAX_WORDS || 28));
  }

  // Guardar memoria (seguimiento + biblia)
  if (!ACK_MODE && key !== "general") {
    // Tomamos el message como última guía/consuelo
